...
/**
 * 按角色名切换当前选中角色，支持来自首页跳转参数或别名的角色键。
 * @param name 目标角色名称。
 * @returns 如果成功匹配并切换则返回 true。
 */
const applySelectedCharacterByName = async (name: string) => {
  const normalizedName = normalizeCharacterName(name)
  if (!normalizedName) return false

  const index = names.value.findIndex((characterName) => characterName === normalizedName)
...
/**
 * 读取当前路由 query 中指定的角色名。
 * @returns 归一化后的角色键名；如果没有提供则返回空字符串。
 */
const getRouteCharacterName = () => {
  const characterQuery = route.query.character
  const rawName = Array.isArray(characterQuery)
    ? (characterQuery[0] ?? '')
    : (characterQuery ?? '')
...
const restoreSelectedCharacter = async () => {
  ...
  if (routeCharacterName && (await applySelectedCharacterByName(routeCharacterName))) {
    return
  }

  const savedCharacterName = getSavedCharacterName()
  if (savedCharacterName && (await applySelectedCharacterByName(savedCharacterName))) {
    return
  }
